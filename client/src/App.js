...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      // listen to the event on line 46
  ...
  listenToPaymentEvent = () => {
    ...
    this.itemManager.events.SupplyChainStep().on("data", async function(evt) {
      ...
      alert("Item " + itemObj._identifier + " was paid. Initiate delivery");
    });
  }

  // this is a react function - can be found on react website - this implements the input changes in the render function
  ...
  // handle submit function in render function - async works with promises - need the cost and itemname so it can be sent to the blockchain
  handleSubmit = async() => {
    const {cost, itemName} = this.state;
    // sends to blockchain - promise delivered
